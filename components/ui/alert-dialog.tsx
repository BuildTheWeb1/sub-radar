  ...
  return (
    <AlertDialogPortal>
      ...
      <AlertDialogPrimitive.Popup
        ...
        className={cn(
          "fixed top-1/2 left-1/2 z-50 w-[calc(100%-2rem)] max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-lg border border-brand-surface-border bg-background p-5 shadow-lg transition duration-150 data-ending-style:scale-95 data-ending-style:opacity-0 data-starting-style:scale-95 data-starting-style:opacity-0",
          className
        )}
        {...props}
      />
...
function AlertDialogTitle({ className, ...props }: AlertDialogPrimitive.Title.Props) {
  return (
    <AlertDialogPrimitive.Title
      ...
      className={cn("text-base font-semibold text-brand-text-strong", className)}
      {...props}
    />
  )
}
